...
以下は旗艦防御力の近似値である。原則切り上げであるがfloat型で計算を行うため整数部が100万を超えたあたりで小数部が丸め誤差を起こす。

<form action="#" method="get" class="inline-grid grid2-auto-fr" oninput="$('#kikan-bougyo').text(floatceil((parseInt($('#job-soukouhei').val()) + parseInt($('#jukuren-senkan').val())) * Math.max(parseInt($('#jukuren-senkan').val()) / 100, 1)).toLocaleString())">
...
以下は僚艦・艦載機防御力の近似値である。原則切り上げであるがfloat型で計算を行うため整数部が100万を超えたあたりで小数部が丸め誤差を起こす。

<form action="#" method="get" class="inline-grid grid2-auto-fr" oninput="$('#ryoukan-bougyo').text(floatceil((stepover100(parseInt($('#job-boukuusirei').val())) + parseInt($('#jukuren-dummy').val())) * Math.max(parseInt($('#jukuren-dummy').val()) / 100, 1)).toLocaleString())">
...
爆発ダメージは防御艦僚艦配備スキルがゼロになる。
ダメージ補正には敵毎に設定されているレベル補正、宇宙闘技場のランク補正がある。
...
<form action="#" method="get" id="kantuu" class="inline-grid grid2-auto-fr" oninput="kantuu()">
...
const kantuu = () => {
	const kantuutbl = $("#kantuu").next("table");
	const kantuutbd = kantuutbl.children("tbody");
	
	const damage   = $("#damage3");
	const cyclops  = $("#cyclops3");
	const cut      = $("#cut3");
	const armor    = $("#armor3");
	
	let   damage_v   = parseInt(damage.val());
	const cyclops_v  = parseInt(cyclops.val());
	const cut_v      = parseInt(cut.val());
	const armor_v    = parseInt(armor.val());
	
	kantuutbd.empty();
	for(let i = 1; i <= 20 && damage_v > 0; i++)
	{
		damage_v = Math.min(9999999, Math.ceil(damage_v * (100 - cut_v) / 100));
		const tr = $(`<tr><td style="text-align: right">${ i }体目</td><td style="text-align: right">${ damage_v.toLocaleString() }</td><td>${ damage_v >= armor_v ? "撃破" : "貫通" }</td></tr>`);
		kantuutbd.append(tr);
		if(cyclops_v == 0)
		{
			damage_v -= armor_v;
