...
```
与ダメージ = [ダメージ * (100 - 敵艦カット率) / 100] + 光拳加算
ダメージ = 基礎攻撃力 + 加算攻撃力
...
次の限界カット率以上の敵はダメージカンストすることができない。
...
| 星生炉縮退砲                     |   25,000 |         98.1 |
| クラウ・ソラス                   |   19,999 |         97.5 |
| 2018式発掘光線砲                 |   18,000 |         97.3 |
| 隕石爆弾                         |   12,000 |         95.9 |
| 拡散超質量氷塊弾など(同威力多数) |    9,999 |           95 |
| 光子フィン                       |    7,800 |         93.6 |
| 光爆防御弾3                      |    4,000 |         87.6 |
| 光輪3                            |    2,200 |         77.3 |

実弾、Eカットの低い方の武装で挑めればよいが、両方のカット率が限界カット率以上の敵も存在する。
...
const kikan = () => {
	...
	const result_v     = Math.min(9999999, Math.ceil(damage_v * (100 - parseFloat(cut.value)) / 100));
...
const kantuu = () => {
	const kantuutbl = document.getElementById("kantuu").nextSibling.nextSibling;
