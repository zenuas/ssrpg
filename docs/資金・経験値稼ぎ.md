...
const sikinkeiken = () => {
	...
	const calc = (name, data, kyori) => {
		...
		$.each(data, (_, stage) => {
			...
			moneyp      += (stage.zmo / stage.znm * Math.ceil(1 + zako_lv / 10) + money)   * gekiha * 0.26 + (stage.bmo * 2 * Math.ceil(1 + boss_lv / 10) + money)   * 0.81;
			keikenp     += (stage.zex / stage.znm * Math.ceil(1 + zako_lv /  2) + kouseki) * gekiha        + (stage.bex     * Math.ceil(1 + boss_lv /  2) + kouseki);
			populationp += (stage.zpo / stage.znm / 2                           + pod_up)  * gekiha * 0.26 + (stage.bpo / 2                               + pod_up)  * 0.81;
			gekihap     += gekiha;
		});
		return({
